...
registro_eventos = []  # aquí guardaremos todos los eventos
...
while True:
    ...
    # 2.1. Botón de exportar CSV
    export_button_x = button_x - 130
    export_button_y = 10
    export_button_w = 120
    export_button_h = 25

    cv2.rectangle(frame, 
                (export_button_x, export_button_y), 
                (export_button_x + export_button_w, export_button_y + export_button_h), 
                (0, 128, 255), cv2.FILLED)
    cv2.putText(frame, "EXPORTAR", 
                (export_button_x + 5, export_button_y + 18), 
    ...
    if is_mouse_down and mouse_x != -1 and mouse_y != -1:
        ...
        elif export_button_x <= mouse_x <= export_button_x + export_button_w and export_button_y <= mouse_y <= export_button_y + export_button_h:
            if registro_eventos:
                with open("registro_eventos.csv", mode='w', newline='', encoding='utf-8') as archivo:
                    campos = ['tipo', 'genero', 'clase', 'fecha', 'hora', 'archivo']
                    writer = csv.DictWriter(archivo, fieldnames=campos)
                    writer.writeheader()
                    for evento in registro_eventos:
                        writer.writerow(evento)
                print("Registro exportado a 'registro_eventos.csv'")
            else:
